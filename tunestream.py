    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                try: await cur.execute("ALTER TABLE tunestream_guild_settings ADD COLUMN fade_seconds FLOAT DEFAULT 5.0")
                except: pass
                try: await cur.execute("ALTER TABLE tunestream_guild_settings ADD COLUMN fade_curve VARCHAR(20) DEFAULT 'linear'")
                except: pass
                ...
                try: await cur.execute("CREATE INDEX tunestream_track_intelligence_recent_idx ON tunestream_track_intelligence (guild_id, last_played)")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_track_intelligence_requester_idx ON tunestream_track_intelligence (guild_id, last_requester_id, last_played)")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_user_affinity_recent_idx ON tunestream_user_track_affinity (guild_id, user_id, last_requested)")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_smart_recommendations_recent_idx ON tunestream_smart_recommendations (guild_id, created_at)")
                except: pass
