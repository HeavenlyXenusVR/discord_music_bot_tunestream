                ...
                async with conn.cursor() as cur:
                    for guild_id, data in list(playback_tracking.items()):
                        try:
                            ...
                            await cur.execute(
                                "UPDATE tunestream_playback_state SET position_seconds = %s, is_playing = %s, is_paused = %s WHERE guild_id = %s AND bot_name = %s",
...
async def build_autodj_query(cur, guild_id):
    await cur.execute("SELECT title FROM tunestream_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 12", (guild_id,))
...
async def maybe_enqueue_autodj(cur, guild, channel_id):
    ...
    try:
        ...
        history_titles = set()
        await cur.execute("SELECT title FROM tunestream_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 18", (guild.id,))
...
async def init_db():
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("CREATE TABLE IF NOT EXISTS tunestream_playback_state (guild_id BIGINT, bot_name VARCHAR(50), channel_id BIGINT, video_url TEXT, position_seconds INT DEFAULT 0, is_playing BOOLEAN DEFAULT FALSE, is_paused BOOLEAN DEFAULT FALSE, title TEXT, PRIMARY KEY (guild_id, bot_name))")
                try: await cur.execute("ALTER TABLE tunestream_playback_state ADD COLUMN title TEXT")
                except: pass
                try: await cur.execute("ALTER TABLE tunestream_playback_state ADD COLUMN is_paused BOOLEAN DEFAULT FALSE")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE tunestream_guild_settings MODIFY loop_mode VARCHAR(10) DEFAULT 'queue'")
                except: pass
                try: await cur.execute("UPDATE tunestream_guild_settings SET loop_mode = 'queue' WHERE loop_mode IS NULL OR loop_mode NOT IN ('off', 'song', 'queue')")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE tunestream_queue ADD COLUMN bot_name VARCHAR(50)")
                except: pass
                try: await cur.execute("ALTER TABLE tunestream_queue_backup ADD COLUMN bot_name VARCHAR(50)")
                except: pass
                try: await cur.execute("ALTER TABLE tunestream_queue ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                try: await cur.execute("ALTER TABLE tunestream_queue_backup ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                try: await cur.execute("ALTER TABLE tunestream_history ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE tunestream_voice_state ADD COLUMN reconnect_attempts INT NOT NULL DEFAULT 0")
                except: pass
                try: await cur.execute("ALTER TABLE tunestream_voice_state ADD COLUMN last_error TEXT NULL")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_voice_state_rejoin_idx ON tunestream_voice_state (bot_name, desired_connected, last_seen_at)")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_metrics_status_idx ON tunestream_metrics (bot_name, updated_at)")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE tunestream_active_playlists ADD COLUMN channel_id BIGINT DEFAULT NULL")
                except: pass
                ...
                try: await cur.execute("CREATE INDEX tunestream_queue_lookup_idx ON tunestream_queue (guild_id, bot_name, id)")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_queue_backup_lookup_idx ON tunestream_queue_backup (guild_id, bot_name, id)")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_history_guild_played_idx ON tunestream_history (guild_id, played_at)")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_history_requester_idx ON tunestream_history (guild_id, requester_id, played_at)")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_playback_resume_idx ON tunestream_playback_state (bot_name, is_playing, guild_id)")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_playlist_bot_idx ON tunestream_active_playlists (bot_name, guild_id)")
                except: pass
                try: await cur.execute("CREATE INDEX tunestream_user_playlist_lookup_idx ON tunestream_user_playlists (user_id, playlist_name)")
                except: pass
                try: await cur.execute("ALTER TABLE tunestream_playback_state ADD COLUMN bot_name VARCHAR(50) DEFAULT 'tunestream'")
                except: pass
                try: await cur.execute("ALTER TABLE tunestream_active_playlists ADD COLUMN bot_name VARCHAR(50) DEFAULT 'tunestream'")
                except: pass
                try: await cur.execute("ALTER TABLE tunestream_bot_home_channels ADD COLUMN bot_name VARCHAR(50) DEFAULT 'tunestream'")
                except: pass
...
async def _process_queue_inner(guild, channel_id, start_position=0, *, allow_recovery_restore=False):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT volume, loop_mode, filter_mode, transition_mode, custom_speed, custom_pitch, custom_modifiers_left, stay_in_vc FROM tunestream_guild_settings WHERE guild_id = %s", (guild.id,))
                res = await cur.fetchone()
            ...
            async with conn.cursor() as cur:
                for guild_id, data in list(playback_tracking.items()):
                    ...
                    if guild and guild.voice_client and _player_is_playing(guild.voice_client):
                        ...
                        await cur.execute("REPLACE INTO tunestream_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, is_paused, title) VALUES (%s, 'tunestream', %s, %s, %s, TRUE, FALSE, %s)", (guild_id, data['channel_id'], data['url'], pos, data['title']))
...
@bot.tree.command(name="tunestream_main_shuffle", description="Randomize the order of the upcoming queue while keeping the currently playing track untouched.")
async def shuffle(interaction: discord.Interaction):
    if not await is_dj(interaction): return
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, video_url, title, requester_id FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream'", (interaction.guild.id,))
                q = await cur.fetchall()
...
@bot.tree.command(name="tunestream_main_fade", description="Enable or disable smooth fade transitions between tracks for softer playback changes.")
...
async def toggle_fade(interaction: discord.Interaction, mode: str):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("UPDATE tunestream_guild_settings SET transition_mode = %s WHERE guild_id = %s", (mode, interaction.guild.id))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                res = await cur.fetchone()
                if res and res[0] != 'none': return await interaction.response.send_message(embed=discord.Embed(description="❌ **Conflict:** Disable standard Filters via `/tunestream_main_filter none` first.", color=discord.Color.red()), ephemeral=True)
