    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await safe_schema_execute(cur, "ALTER TABLE tunestream_playback_state ADD COLUMN track_uid CHAR(32) DEFAULT NULL")
                ...
                await safe_schema_execute(cur, "ALTER TABLE tunestream_queue ADD COLUMN track_uid CHAR(32) DEFAULT NULL")
                await safe_schema_execute(cur, "ALTER TABLE tunestream_queue_backup ADD COLUMN track_uid CHAR(32) DEFAULT NULL")
                ...
                await safe_schema_execute(cur, "ALTER TABLE tunestream_active_playlist_tracks ADD COLUMN track_uid CHAR(32) DEFAULT NULL")
                ...
                await safe_schema_execute(cur, "CREATE INDEX tunestream_queue_track_uid_idx ON tunestream_queue (guild_id, bot_name, track_uid)")
                await safe_schema_execute(cur, "CREATE INDEX tunestream_queue_backup_track_uid_idx ON tunestream_queue_backup (guild_id, bot_name, track_uid)")
                ...
                await safe_schema_execute(cur, "DROP INDEX tunestream_queue_track_uid_idx ON tunestream_queue")
                await safe_schema_execute(cur, "DROP INDEX tunestream_queue_backup_track_uid_idx ON tunestream_queue_backup")
                ...
                await cur.execute(
                    "UPDATE tunestream_queue SET track_uid = SUBSTRING(SHA2(CONCAT(UUID(), RAND(), id, COALESCE(video_url, ''), COALESCE(title, '')), 256), 1, 32) "
                    "WHERE track_uid IS NULL OR track_uid = ''"
                )
                await cur.execute(
                    "UPDATE tunestream_queue_backup SET track_uid = SUBSTRING(SHA2(CONCAT(UUID(), RAND(), id, COALESCE(video_url, ''), COALESCE(title, '')), 256), 1, 32) "
                    "WHERE track_uid IS NULL OR track_uid = ''"
                )
                await cur.execute(
                    "UPDATE tunestream_active_playlist_tracks SET track_uid = SUBSTRING(SHA2(CONCAT(UUID(), RAND(), guild_id, COALESCE(playlist_url, ''), position_idx, COALESCE(video_url, ''), COALESCE(title, '')), 256), 1, 32) "
                    "WHERE track_uid IS NULL OR track_uid = ''"
                )
                await cur.execute(
                    "UPDATE tunestream_playback_state SET track_uid = SUBSTRING(SHA2(CONCAT(UUID(), RAND(), guild_id, COALESCE(video_url, ''), COALESCE(title, '')), 256), 1, 32) "
                ...
                await safe_schema_execute(cur, "CREATE INDEX tunestream_error_events_guild_time_idx ON tunestream_error_events (guild_id, created_at)")
...
async def get_current_track_snapshot(guild_id):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "SELECT video_url, title, track_uid FROM tunestream_playback_state WHERE guild_id = %s AND bot_name = 'tunestream' AND (is_playing = TRUE OR is_paused = TRUE) ORDER BY is_playing DESC LIMIT 1",
                    (guild_id,),
                )
                row = await cur.fetchone()
    if not row:
...
async def backup_track(cur, guild_id, video_url, title, requester_id, track_uid=None):
    track_uid = _ensure_track_uid(track_uid)
    await cur.execute(
        "SELECT COUNT(*) FROM tunestream_queue_backup WHERE guild_id = %s AND bot_name = 'tunestream' AND track_uid = %s",
        (guild_id, track_uid),
    )
...
async def delete_live_queue_copies(cur, guild_id, video_url, title, track_uid=None):
    ...
    if normalized_uid:
        await cur.execute(
            "DELETE FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream' AND track_uid = %s",
            (guild_id, normalized_uid),
        )
        return max(0, int(getattr(cur, "rowcount", 0) or 0))
    await cur.execute(
        "SELECT id, video_url, title, track_uid FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream'",
        (guild_id,),
    )
    rows = list(await cur.fetchall() or [])
    deleted = 0
...
async def restore_queue_from_backup(cur, guild_id, requester_id=None):
    ...
    for row in rows:
        ...
        insert_data.append((guild_id, 'tunestream', url, title, requester_id if requester_id is not None else backup_requester_id, track_uid))
    if insert_data:
        await cur.executemany(
            """
            INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id, track_uid)
            VALUES (%s, %s, %s, %s, %s, %s)
            ON DUPLICATE KEY UPDATE
                video_url = VALUES(video_url),
                title = VALUES(title),
...
async def repair_queue_backup_parity(cur, guild_id, *, reason="queue_integrity", active_player=None):
    ...
    await cur.execute(
        "SELECT video_url, title, is_playing, is_paused, position_seconds, track_uid FROM tunestream_playback_state WHERE guild_id = %s AND bot_name = 'tunestream' LIMIT 1",
        (guild_id,),
    )
...
async def restore_active_playback_entry(cur, guild_id, requester_id=None):
    await cur.execute(
        "SELECT video_url, title, track_uid FROM tunestream_playback_state "
...
async def requeue_failed_track(cur, guild_id, channel_id, url, title, requester_id, *, position=0, reason="recovery", track_uid=None):
    ...
    async with get_track_requeue_lock(guild_id):
        ...
        if not original_track_uid:
            await cur.execute("SELECT track_uid FROM tunestream_playback_state WHERE guild_id = %s AND bot_name = 'tunestream' LIMIT 1", (guild_id,))
            ...
            try:
                ...
                if normalized_track_uid:
                    await cur.execute(
                        "SELECT video_url, title, requester_id, track_uid FROM tunestream_queue_backup WHERE guild_id = %s AND bot_name = 'tunestream' AND track_uid = %s ORDER BY id ASC LIMIT 1",
                        (guild_id, normalized_track_uid),
                    )
                else:
                    await cur.execute(
                        "SELECT video_url, title, requester_id, track_uid FROM tunestream_queue_backup WHERE guild_id = %s AND bot_name = 'tunestream' AND (video_url = %s OR title = %s) ORDER BY id ASC LIMIT 1",
                        (guild_id, url, title),
                    )
        ...
        if recent_until and now < recent_until:
            ...
            await insert_queue_front(cur, "tunestream_queue", guild_id, "tunestream", url, title, requester_id, track_uid)
            if guild_id in playback_tracking: playback_tracking[guild_id]["track_uid"] = track_uid
            await remember_recovery_state(guild_id, channel_id, position)
        ...
        await cur.execute(
            "UPDATE tunestream_playback_state SET is_playing = FALSE, is_paused = FALSE, position_seconds = %s, track_uid = %s, play_session_key = %s WHERE guild_id = %s AND bot_name = 'tunestream'",
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if not next_song:
                    await cur.execute(
                        "UPDATE tunestream_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE, play_session_key = NULL, track_uid = NULL WHERE guild_id = %s AND bot_name = 'tunestream'",
                        (guild.id,),
                    )
                ...
                try:
                    ...
                    if _is_direct_media_url(url):
                        playback_tracking.pop(guild.id, None)
                        await cur.execute(
                            "UPDATE tunestream_playback_state SET video_url = NULL, title = NULL, is_playing = FALSE, is_paused = FALSE, position_seconds = 0, play_session_key = NULL, track_uid = NULL WHERE guild_id = %s AND bot_name = 'tunestream'",
                            (guild.id,),
                        )
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                insert_data = [(interaction.guild.id, 'tunestream', track.uri, track.title, interaction.user.id, _new_track_uid())]
                insert_data.extend((
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("UPDATE tunestream_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE, play_session_key = NULL, track_uid = NULL WHERE guild_id = %s AND bot_name = 'tunestream'", (interaction.guild.id,))
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if rows:
                    try:
                        await cur.execute("START TRANSACTION")
                        for _sk_row in rows: await cur.execute("DELETE FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream' AND track_uid = %s LIMIT 1", (interaction.guild.id, _row_track_uid(_sk_row, 2)))
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "DELETE FROM tunestream_smart_recommendations WHERE created_at < NOW() - INTERVAL 90 DAY"
                    )
                    await cur.execute(
                        "DELETE FROM tunestream_error_events WHERE created_at < NOW() - INTERVAL 30 DAY"
                    )
                    await cur.execute(
                        "DELETE FROM tunestream_track_intelligence WHERE last_played < NOW() - INTERVAL 365 DAY AND play_count <= 1 AND finish_count = 0"
                    )
                    await cur.execute(
                ...
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor(aiomysql.DictCursor) as cur:
                            ...
                            await cur.execute(
                                """
                                UPDATE tunestream_swarm_direct_orders
                                ...
                                LIMIT %s
                                """,
                                (current_claim_token, 'tunestream', DIRECT_ORDER_MAX_ATTEMPTS, DIRECT_ORDER_CLAIM_TIMEOUT_SECONDS, DIRECT_ORDER_FETCH_LIMIT),
                            )
...
async def queue_integrity_check_loop():
    ...
    was dequeued (DELETE FROM tunestream_queue) at play-start but the bot crashed or
    disconnected before it finished, so it vanished from both the live queue and the
    player, yet the backup still has a copy.

    For each guild where tunestream_playback_state reports a track was playing but
    tunestream_queue is now empty and the player is not active, this loop locates that
    specific track in tunestream_queue_backup and re-inserts it at the front of the live
            ...
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for row in candidates:
                        ...
                        await cur.execute(
                            "SELECT video_url, title, requester_id, track_uid FROM tunestream_queue_backup "
                            "WHERE guild_id = %s AND bot_name = 'tunestream' "
